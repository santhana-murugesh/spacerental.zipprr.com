...
@section('content')
          ...
          <div class="card-header-area">
              ...
              <div class="card-header-buttons">
                ...
                <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                  data-href="{{ route('admin.hotel_management.category.bulk_delete') }}">
                    ...
                    <tbody>
                      @foreach ($categories as $category)
                        <tr>
                          ...
                          <td>
                            @if($category->icon)
                              @if(str_contains($category->icon, '<svg') || str_contains($category->icon, '<SVG'))
                                <div class="category-icon" style="width: 30px; height: 30px;">
                                  {!! $category->icon !!}
                                </div>
                              @elseif(str_contains($category->icon, 'fas ') || str_contains($category->icon, 'fa ') || str_contains($category->icon, 'fab ') || str_contains($category->icon, 'far '))
                                <i class="{{ $category->icon }}" style="font-size: 24px;"></i>
                              @else
                                <span class="category-icon" style="font-size: 24px;">{{ $category->icon }}</span>
                              @endif
                            @else
                          ...
                          <td>
                            ...
                            <form class="deleteForm d-inline-block"
                              action="{{ route('admin.hotel_management.category.delete', ['id' => $category->id]) }}"
                              method="post">
                              @csrf
  ...
  {{-- create modal --}}
  @include('admin.hotel-management.category.create')

  {{-- edit modal --}}
  @include('admin.hotel-management.category.edit')
@endsection
