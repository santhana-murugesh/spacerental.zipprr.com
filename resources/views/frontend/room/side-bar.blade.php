<div class="offcanvas-body p-0">
  <aside class="widget-area widget-area-style-2 px-20">
    ...
    <div class="widget widget-booking-hour py-20">
      <h5 class="title">
      ...
      <div id="bookinghours" class="collapse show">
        <div class="mt-20" data-toggle-list="bookinghours" data-toggle-show="4">
          <ul class="list-group list-group-bordered mb-2">
            <li class="list-group-item">
              <div class="form-check p-0">
                <label class="form-check-label mb-0" for="radio_all">
                  <input class="form-check-input ml-0 hour" type="radio" name="charge" id="radio_all" value=""
                    checked>
                  {{ __('All') }}
                </label>
              </div>
                ...
                <div class="form-check p-0">
                  <label class="form-check-label mb-0" for="radio_{{ $charge->id }}">
                    <input class="form-check-input ml-0 hour" type="radio" name="charge"
                      id="radio_{{ $charge->id }}" value="{{ $charge->hour }}">
    ...
    <div class="widget widget-price py-20">
      ...
      <div id="options" class="collapse show">
        <div class="mt-20">
          ...
          <div class="col-md-12 col-xxl-12">
            <div class="form-group select-area mb-20">
              <select class="form-control select2 hotelDropdown" id="hotelDropdown">
                <option value="" selected disabled>{{ __('Select Hotel') }}</option>
          ...
          <div class="form-group mb-20">
            <select class="form-control select2 adultDropdown">
              <option value="" selected disabled>{{ __('Select Adult') }}</option>
              <option value="">{{ __('All') }}</option>
              @for ($i = 1; $i <= $adultNumber; $i++)
                <option @if (request()->input('adult') == $i) selected @endif value="{{ $i }}">
                  {{ $i }}
                </option>
              @endfor
            </select>
          ...
          <div class="form-group mb-20">
            <select class="form-control select2 childrenDropdown">
              <option value="" selected disabled>{{ __('Select Children') }}</option>
              <option value="">{{ __('All') }}</option>
              @for ($i = 0; $i <= $childrenNumber; $i++)
                <option @if (request()->input('children') == $i) selected @endif value="{{ $i }}">
                  {{ $i }}
                </option>
              @endfor
            </select>
              ...
              <div id="amenities" class="collapse show">
                <div class="accordion-body">
                  <ul class="list-group custom-checkbox toggle-list" data-toggle-list="amenitiesToggle"
                    data-toggle-show="5">
                    @php
                      $amenitie = request()->input('amenitie');
                      $hasamenitie = explode(',', $amenitie);
                    @endphp

                    @foreach ($amenities as $amenitie)
                      @if (in_array($amenitie->id, $hasamenitie))
                        <li>
                          <input class="input-checkbox" type="checkbox" name="checkbox"
                            id="checkbox_{{ $amenitie->id }}" value="{{ $amenitie->id }}" checked>
                          <label class="form-check-label"
                            for="checkbox_{{ $amenitie->id }}"><span>{{ $amenitie->title }}</label>
                        </li>
                      @else
                        <li>
                          <input class="input-checkbox" type="checkbox" name="checkbox"
                            id="checkbox_{{ $amenitie->id }}" value="{{ $amenitie->id }}">
                          <label class="form-check-label"
                            for="checkbox_{{ $amenitie->id }}"><span>{{ $amenitie->title }}</span></label>
                        </li>
                      @endif
                    @endforeach
                  </ul>
    ...
    <div class="cta mb-30">
      <a href="{{ route('frontend.rooms') }}" class="btn btn-lg btn-primary icon-start w-100"><i
