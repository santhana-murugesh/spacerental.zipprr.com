...
function initStickyNavigation() {
  ...
  const header = document.querySelector('.sticky-header-nav');
  ...
  // Also handle on page load in case user refreshes while scrolled
  const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
  if (scrollTop > 50) {
    header.classList.add('sticky');
  }
  
  // Ensure header stays visible on window resize
