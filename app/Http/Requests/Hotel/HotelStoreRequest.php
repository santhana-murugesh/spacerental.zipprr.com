...
class HotelStoreRequest extends FormRequest
{
    ...
    public function rules(Request  $request)
    {
        if ($request->vendor_id == 0) {
            ...
            if ($packagePermission != []) {
                ...
                $rules = [
                    'slider_images' => 'required|array|max:' . $hotelImageLimit,
                    'logo' => [
                        'required',
                        new ImageMimeTypeRule(),
                        'dimensions:width=300,height=300'
    ...
    public function messages()
    {
        ...
        foreach ($days as $day) {
            $dayName = ucfirst($day);
            $messageArray[$day . '_slots.*.start.required'] = __('The start time is required for :day slots.', ['day' => $dayName]);
            $messageArray[$day . '_slots.*.start.date_format'] = __('The start time must be in HH:MM format for :day slots.', ['day' => $dayName]);
            $messageArray[$day . '_slots.*.end.required'] = __('The end time is required for :day slots.', ['day' => $dayName]);
            $messageArray[$day . '_slots.*.end.date_format'] = __('The end time must be in HH:MM format for :day slots.', ['day' => $dayName]);
            $messageArray[$day . '_slots.*.end.after'] = __('The end time must be after the start time for :day slots.', ['day' => $dayName]);
        }
        foreach ($languages as $language) {
            $code = $language->code;
